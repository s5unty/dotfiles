...
-- Create a console
function CsshConsole:new(config)
   ...
   config.name     = config.name     or "CsshConsoleNeedsUniqueName" -- window name
   ...
   local console = setmetatable(config, { __index = CsshConsole })
   ...
   -- "Reattach" currently running CsshConsole. This is in case awesome is restarted.
...
-- Toggle the console
function CsshConsole:toggle()
   self.visible = not self.visible
   self:display()
end

setmetatable(_M, { __call = function(_, ...) return CsshConsole:new(...) end })
