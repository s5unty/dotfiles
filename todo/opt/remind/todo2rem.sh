...
#
# 功能：
#   [todotxt][1] [文本文件][2]发生变更时(依赖 inotifywait)、
#   处理其中含有 **提醒标记** 的任务事项，
#   把他们格式化成 [remind][3] [格式][4] 的纯文本，
#   并把这些格式化后的纯文本另存为 remind 数据文件。
#
...
# $notify_program 用于通知的外挂程序、支持手机提醒(pushover.net)和桌面提醒(notify-send)
#
#   1. 下午两点开会
#
#     T: 下午两点开会 @106 rem:(13:30)
#     R: REM AT 13:30 RUN $notify_program "%1" "下午两点开会 @106"
#     N: 现在：下午两点开会 @106                                        <= 13:00收到第一次也是最后一次通知
#
#   2. 下午两点开会，提前半小时提醒
#
#     T: 下午开会 @106 rem:(14:00 +30)
#     R: REM AT 14:00 +30 RUN $notify_program "%1" "下午开会 @106"
#     N: 30分钟后：下午开会 @106                                        <= 13:30收到第一次通知
#     N: 现在：下午开会 @106                                            <= 14:00收到最后一次通知
#
#   3. 26号下午三点开会，提前半小时和五分钟，各提醒一次
#
#     T: 又开会 @504 rem:(26 at 15:00 +30 *25)
#     R: REM 26 AT 15:00 +30 RUN $notify_program "%1" "又开会 @504"
#     N: 30分钟后：又开会 @504                                          <= 26号下午14:30收到第一次通知
#     N: 5分钟后：又开会 @504                                           <= 26号下午14:55收到最后一次通知
#
#   4. 下午有课
#
#     T: 上课 @601 rem:(mon, tue, fri at 17:00 +5)
#     R: REM mon, tue, fri AT 17:00 +5 RUN $notify_program "%1" "上课 @601"
#     N: 5分钟后：上课 @601                                             <= 星期一、二、五的下午16:55收到一次通知
#
#   5. 续费提醒
#
#     T: 域名到期 rem:(2012-12-26 +2)
#     R: REM 2012-12-26 +2 RUN $notify_program "%b" "域名到期"
...
#     R: REM 26 Dec [$Uy] -15 *7 UNTIL 26 Dec [$Uy] RUN $notify_program [dosubst("%b%", date($Uy, $Um, 26))] "谁的[$Uy - 1955]岁生日"
...
TODO_DIRECTORY="/sun/.todo"
DATA_DIRECTORY="$TODO_DIRECTORY/var"
NOTIFY_PROGRAM="$TODO_DIRECTORY/opt/remind/notify-wm"
#NOTIFY_PROGRAM="$TODO_DIRECTORY/opt/remind/pushover.sh"

for TODO_FILE in $TODO_DIRECTORY/**/todo.log; do
    ...
    while read line; do
        ...
        # 删除 todotxt 格式的时间戳标记
        line=$(echo $line | sed -e 's/[ ]\+[tuxs]:[0-9]\{2,4\}[-/.][0-9]\{1,2\}[-/.][0-9]\{1,2\}[ ]\+/ /g')
        ...
        if [[ $spec =~ $date_time_spec ]]; then
            # 日期＋时间
            rem=$(echo $line | sed -e 's,\(.*\)rem:(\(.*\)),REM \2 RUN '$NOTIFY_PROGRAM' "%1" "\1 (%2)",g' | sed -e 's, at ,\U&,g')
        elif [[ $spec =~ $only_time_spec ]]; then
            # 只有时间
            rem=$(echo $line | sed -e 's,\(.*\)rem:(\(.*\)),REM AT \2 RUN '$NOTIFY_PROGRAM' "\%1" "\1 (%2)",g')
        else
            # 只有日期
            ...
            rem=$(echo $line | sed -e 's,\(.*\)rem:(\(.*\)),REM \2 AT 00:00 RUN '$NOTIFY_PROGRAM\ \"$sub\"' "\1",g')
        fi
